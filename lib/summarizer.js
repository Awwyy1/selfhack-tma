...
export async function loadConversationWithSummaries(userId, recentLimit = 50) {
  ...
  // 3. Форматируем саммари для системного промпта
  let summaryContext = '';
  if (summaries && summaries.length > 0) {
    summaryContext = '\n\n=== ИСТОРИЯ РАЗГОВОРОВ ===\n';
    summaryContext += 'Вот краткое содержание предыдущих разговоров с пользователем:\n\n';
    summaries.forEach(s => {
      summaryContext += `[Сообщения ${s.block_start}-${s.block_end}]: ${s.summary}\n\n`;
    });
    summaryContext += 'Используй эту информацию как контекст о пользователе. Помни его цели, проблемы и прогресс.';
  }

  // 4. Формируем массив сообщений (только реальная история)
  const messages = [];
